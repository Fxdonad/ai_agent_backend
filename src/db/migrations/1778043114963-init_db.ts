...
export class InitDb1778043114963 implements MigrationInterface {
    name = 'InitDb1778043114963'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`messages\` (\`id\` varchar(36) NOT NULL, \`role\` enum ('system', 'user', 'assistant') NOT NULL, \`content\` longtext NOT NULL, \`command\` longtext NULL, \`output\` longtext NULL, \`metadata\` json NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`sessionId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
